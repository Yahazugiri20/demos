...
6. **Update layout.tsx** to wrap app with Providers (REQUIRED — providers only work when wrapping the app)
7. Handle SafeArea (see conditional workflow below)
8. Update manifest: `frame` → `miniapp` — see [references/MANIFEST.md](references/MANIFEST.md)
9. Rename config: `minikit.config.ts` → `farcaster.config.ts`, `minikitConfig` → `farcasterConfig`, update import in `.well-known` route
