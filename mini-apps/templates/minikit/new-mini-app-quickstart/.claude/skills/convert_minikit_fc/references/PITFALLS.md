...
- [Runtime Issues](#runtime-issues)
  ...
  - Context is null / layout.tsx not updated
  - Failed to fetch
...
MiniAppProvider not in provider chain, or layout.tsx not updated.

**Common mistake**: Creating provider files but not updating `layout.tsx` to use them.

```typescript
// WRONG - layout.tsx still using old setup or no providers
export default function RootLayout({ children }) {
  return <html><body>{children}</body></html>;
}

// CORRECT - layout.tsx wraps children with Providers
...
Also ensure providers.tsx has MiniAppProvider:
