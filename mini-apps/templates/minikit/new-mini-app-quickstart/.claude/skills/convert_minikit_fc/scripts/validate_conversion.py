...
# Match quoted property names followed by colon (JSON/TS object syntax)
...
# Check if Providers wrapper is used in layout
PROVIDERS_WRAPPER = re.compile(r'<Providers[^>]*>.*</Providers>', re.DOTALL)
PROVIDERS_IMPORT = re.compile(r"from\s+['\"]\.\/providers['\"]")
...
def check_layout_uses_providers(project_path: Path, result: ValidationResult):
    """Check that layout.tsx wraps children with Providers."""
    layout_paths = [
        project_path / 'src' / 'app' / 'layout.tsx',
        project_path / 'app' / 'layout.tsx',
        project_path / 'src' / 'app' / 'layout.js',
        project_path / 'app' / 'layout.js',
    ]
    ...
    for layout_path in layout_paths:
        if layout_path.exists():
            try:
                ...
                rel_path = layout_path.relative_to(project_path)

                has_import = PROVIDERS_IMPORT.search(content)
                has_wrapper = PROVIDERS_WRAPPER.search(content)

                if has_import and has_wrapper:
                    result.add_pass(f"Providers wrapper used in layout: {rel_path}")
                elif has_import:
                    result.add_warning(f"Providers imported but may not be wrapping children: {rel_path}")
                else:
                    result.add_error(f"Layout must import and use Providers to wrap children: {rel_path}")
                return
            except Exception:
                pass

    result.add_warning("No layout.tsx found - ensure providers wrap the app")
...
def validate_project(project_dir: str) -> ValidationResult:
    ...
    check_layout_uses_providers(project_path, result)
